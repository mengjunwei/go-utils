...
func (e *Discovery) discovery() error {
	...
	log.InfoF("%s is add to watching.", e.addr)
	return nil
}

func (e *Discovery) startWatch() {
	e.watchCount += 1
	log.InfoF("discovery watch count: %d", e.watchCount)
...
func (e *Discovery) watchEvent(ech <-chan zk.Event) {
	...
	count := e.watchCount
	log.DebugF("discovery watchEvent begin, watch count: %d", count)
	...
	case event := <-ech:
		...
		if event.Type == zk.EventNodeChildrenChanged {
			log.DebugF("discovery watch action...")
			e.startWatch()
		}
	...
	log.DebugF("discovery watchEvent quit, watch count: %d", count)
}
...
func (e *Discovery) callbackDelayLoop() {
	...
	for {
		...
		case event := <-e.cbChan:
			...
			log.DebugF("discovery callbackDelayLoop cb will  be call: %v, now:%d, delay:%d s", event, now, e.cbDelay)
